...
(function(){
  ...
  function openAIAfterAuth(){
    ...
    const open=()=>{if(typeof window.venomOpenChat==='function'){sessionStorage.removeItem('venom-open-ai');window.venomOpenChat();return true}if(typeof window.newChat==='function'){sessionStorage.removeItem('venom-open-ai');window.newChat();return true}return false};
    if(open())return;let attempts=0;const retry=()=>{if(open()||++attempts>=20)return;setTimeout(retry,150)};setTimeout(retry,150);
  }
  async function boot(){
    const results=await Promise.all(modules.map(([src,attr])=>load(src,attr)));
    const failed=results.filter(r=>!r.ok).map(r=>r.src);if(failed.length)console.warn('[VENOM RUNTIME] optional modules unavailable:',failed);
    removePause();
