...
(function(){
  ...
  function skyline(){
    ...
    const buildings=[
      ['ESB','empire',43,330],['MIDTOWN', 'tower',18,215],['CHRYSLER','spire',61,245],['HUDSON','block',76,165],['BROOKLYN','block',7,125]
    ];
    ...
    const title=document.createElement('div');title.className='vgCityLabel';title.textContent='NEW YORK // NIGHT SIGNAL';wrap.appendChild(title);
    panel.appendChild(wrap);
  }
  function cleanMap(){
    const map=document.querySelector('.vgMap'); if(!map||map.dataset.cleaned)return;
    ...
    const layer=document.createElement('div');layer.className='vgLandmarks';
    landmarks.forEach(([n,x,y])=>{const e=document.createElement('span');e.textContent=n;e.style.left=x;e.style.top=y;layer.appendChild(e)});
    map.appendChild(layer);
    const blocks=document.createElement('div');blocks.className='vgBlocks';
    for(let i=0;i<28;i++){const b=document.createElement('i');b.style.left=(5+(i%7)*12)+'%';b.style.top=(10+Math.floor(i/7)*20)+'%';blocks.appendChild(b)}
    map.appendChild(blocks);
  }
  function boot(){skyline();cleanMap();}
