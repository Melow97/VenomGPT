...
Then copy the Google Client ID + Client Secret into the Supabase Dashboard under Authentication → Providers → Google.

Do not commit the Client Secret to GitHub. Put server-only secrets in Cloudflare environment variables.

## Venom auth behavior
After Supabase authentication, the database trigger assigns the configured owner account to role=owner and plan=pro. All other new users receive role=user and plan=free.
